...
MAX_LEVEL = 15                          -- What level should we let heroes get to?
USE_CUSTOM_XP_VALUES = true             -- Should we use custom XP values to level up heroes, or the default Dota numbers?
CUSTOM_HERO_RESPAWN = true
HERO_RESPAWN_TIME = 5

-- Fill this table up with the required XP per level if you want to change it
XP_PER_LEVEL_TABLE = {}
XP_PER_LEVEL_TABLE[0] =0
XP_PER_LEVEL_TABLE[1] =2
for i=2,4 do
  XP_PER_LEVEL_TABLE[i] = XP_PER_LEVEL_TABLE[i-1] + 2
end

for i=5,9 do
  XP_PER_LEVEL_TABLE[i] = XP_PER_LEVEL_TABLE[i-1] + 4
end

for i=10,13 do
  XP_PER_LEVEL_TABLE[i] = XP_PER_LEVEL_TABLE[i-1] + 8
end
...
--	DeepPrintTable(keys)
	...
	if npc:IsRealHero() and npc.bFirstSpawned == nil and name~="npc_dota_hero_treant" then
...
--				347624347,-- Vitalya Medvedev(vulkan)
				...
				179496404,-- Maxim Chermat(Shesmu)
			...
			for _,ID in pairs(roshdefAdmin) do
				if steamID == ID then
					npc:AddNewModifier( npc, nil, "modifier_special_effect_donator", {duration = -1})
				end
			end
			
			addItemBySteamID(roshdefAdmin,"item_roshan_essence",steamID,npc)
			addItemBySteamID(roshdefAdmin,"item_skull_of_midas",steamID,npc)
...
function GameMode:OnItemPickedUp(keys)
	...
	if itemname == "item_chicken_coin" then
		...
		SendOverheadEventMessage( heroEntity, OVERHEAD_ALERT_GOLD, heroEntity, r, nil )
...
function GameMode:OnEntityKilled( keys )
	...
	if killedUnit:IsRealHero() and CUSTOM_HERO_RESPAWN == true then
